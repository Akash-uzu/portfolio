...
const AiChat: React.FC = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const result: any = await analyzeResume(query, RESUME_TEXT);
      setResponse(result);
    } catch (err: any) {
    ...
    <section id="ai-analyst" className="py-16">
      <div className="bg-gray-800/50 rounded-lg p-6 md:p-8 border border-gray-700 shadow-xl">
        ...
        <p className="text-gray-400 mb-6">
          Ask my AI assistant a question about my resume. Powered by Google
          Gemini.
        </p>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {exampleQuestions.map((q) => (
              <button
                ...
                className="text-sm bg-gray-700 hover:bg-gray-600 text-gray-300 px-2 py-1 rounded-md transition"
              >
                "{q}"
              </button>
            ))}
